    ...
    <div class="card-body">
      <table id="mytable" class="table table-bordered table-hover">
        ...
        <tbody>
          ...
          WHERE i.id_pengirim=? GROUP BY no_pengajuan DESC";
          ...
          // die();
          if ($stmt->rowCount() > 0) {
            $selectSql = "SELECT p.*, i.*, d.*, k1.nama nama_pengirim, k2.nama nama_verifikator , SUM(i.ontime) total_ontime, SUM(i.bongkar) total_bongkar FROM pengajuan_insentif_borongan p
          INNER JOIN insentif i on p.id_insentif = i.id
          LEFT JOIN karyawan k1 on i.id_pengirim = k1.id
          LEFT JOIN karyawan k2 on p.id_verifikator = k2.id
          INNER JOIN distribusi d on i.id_distribusi = d.id
